# Y&ARCHER WORKS 인증 및 권한 설계 가이드

> [2026-07-04] 아키텍처 개편 반영: 내부 통합 앱 Y&ARCHER WORKS(works.ynarcher.co.kr) + 외부 포털 Y&ARCHER WORKS-GUEST(guest.ynarcher.co.kr, Phase 2) 2앱 체제로 전환, 권한 도메인 키 dev→admin, work→ac 교체.

본 문서는 Y&ARCHER WORKS의 인증, 권한, 접근 제어 기준을 정의한다. Y&ARCHER WORKS는 하나의 내부 통합 앱 안에서 7개 섹션을 제공하며, 사용자는 하나의 계정 체계로 로그인하고 섹션별 권한에 따라 접근한다.

권한 설계의 핵심 질문은 다음과 같다.

> 누가, 어떤 섹션에 들어갈 수 있고, 어떤 데이터를 보고, 무엇을 수정할 수 있는가?

## 1. 기본 원칙

Y&ARCHER WORKS의 권한은 단순한 `admin/user` 구분으로 처리하지 않는다. 사용자 유형, 섹션 도메인 권한, 데이터 범위를 함께 판단한다.
...
Domain Permission  = 섹션별 read/write 권한
Data Scope         = 해당 섹션 안에서 접근 가능한 데이터 범위
```
...
```txt
이메일 기반 로그인
하나의 계정으로 WORKS의 모든 섹션 접근
WORKS 단일 도메인에서 동일한 세션 체계 사용
권한은 로그인 이후 admin 스키마의 권한 테이블에서 조회
```

앱 도메인 (ynarcher.co.kr의 서브도메인):

```txt
works.ynarcher.co.kr   -> Y&ARCHER WORKS (내부 통합 앱, 7개 섹션)
guest.ynarcher.co.kr   -> Y&ARCHER WORKS-GUEST (외부 포털, Phase 2)
```

접근 흐름:

```txt
1. 사용자가 WORKS 앱(works.ynarcher.co.kr)에 접속한다.
2. 세션이 없으면 로그인 페이지로 이동한다.
3. 로그인 성공 후 원래 접근하려던 섹션으로 redirect한다.
4. 앱은 admin 권한 테이블에서 해당 섹션 권한을 조회한다.
...
섹션별 권한은 `none`, `read`, `write`를 기본으로 한다.
...
도메인 권한만으로는 충분하지 않다. 같은 `ac:read`라도 사용자 유형에 따라 볼 수 있는 데이터 범위가 달라진다.
...
참가 스타트업:
ac:write, scope=company, scope_id={startup_id}
```
...
| 템플릿 | Hub | Admin | AC | M&A | Project | Fund | Management |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
WORKS 앱(apps/works)은 섹션 진입 시 해당 도메인 권한을 확인한다.

```txt
HUB 섹션              -> domain_name = hub
관리(ADMIN) 섹션      -> domain_name = admin
AC 섹션               -> domain_name = ac
M&A 섹션              -> domain_name = mna
PROJECT 섹션          -> domain_name = project
FUND 섹션             -> domain_name = fund
MANAGEMENT 섹션       -> domain_name = management
```
...
```txt
이 섹션에 접근할 권한이 없습니다.
필요한 경우 관리(ADMIN) 섹션 관리자에게 권한을 요청하세요.
```
...
구현(Phase 1.4): claim 주입은 **Custom Access Token Hook** `admin.custom_access_token_hook`이 담당한다. 토큰 발급/갱신 시 `admin.user_permissions`를 읽어 `app_metadata.permissions`(도메인별 read/write/scope/expires_at)와 `app_metadata.roles`(역할 배열)를 주입하며, `supabase/config.toml`의 `[auth.hook.custom_access_token]`에 `pg-functions://postgres/admin/custom_access_token_hook`로 등록한다(원격은 대시보드 Auth Hooks에서도 활성화). RLS helper는 `admin.can_read_domain`/`admin.can_write_domain`/`admin.get_scope_type`/`admin.get_scope_id`/`admin.has_role`/`admin.is_master`/`admin.can_merge_master`로 제공한다.
...
```sql
CREATE POLICY "ac programs read"
ON ac.programs
FOR SELECT
TO authenticated
USING (
    admin.can_read_domain('ac')
);
```
...
외부 사용자는 WORKS(내부 앱)에 접근할 수 없으며, WORKS-GUEST 앱(Phase 2)만 사용한다. guest의 데이터 접근은 GUEST 앱이 별도 view/RPC로 제공하며, 내부 데이터에는 접근할 수 없다.
...
```txt
관리(ADMIN) 섹션 접근 권한은 master 계열에게만 부여한다.
외부 사용자는 WORKS(내부 앱)에 접근할 수 없고, WORKS-GUEST 앱만 사용한다.
섹션별 read/write 권한과 RLS 정책은 항상 함께 관리한다.
...
Y&ARCHER WORKS의 권한 모델은 다음 조합으로 설계한다.
...
사업부는 AC/Project 중심 쓰기 접근
외부 전문가는 본인 배정 업무만 접근
참가 스타트업은 자기 회사 데이터만 접근
관리자는 관리(ADMIN) 섹션에서 권한을 부여하고 회수
```
