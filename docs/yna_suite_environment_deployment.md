# Y&A Suite 환경변수 및 배포 환경 전략 가이드

본 문서는 Y&A Suite의 local/dev/staging/production 환경 구성, 앱별 환경변수, Supabase 프로젝트, Vercel 배포, 도메인 및 인증 callback 전략을 정의한다.

Y&A Suite는 하나의 모노레포에서 여러 앱을 관리하고, 각 앱을 독립 도메인으로 배포한다.
...
인증 callback URL은 모든 서비스 도메인을 명시적으로 등록한다.
...
각 앱은 별도 Vercel 프로젝트로 배포한다.

```txt
Vercel Project: yna-hub
Root Directory: apps/hub
Domain: hub.ynarcher.co.kr

Vercel Project: yna-dev
Root Directory: apps/dev
Domain: dev.ynarcher.co.kr

Vercel Project: yna-work
...
```txt
hub.stg.ynarcher.co.kr
dev.stg.ynarcher.co.kr
work.stg.ynarcher.co.kr
mna.stg.ynarcher.co.kr
project.stg.ynarcher.co.kr
fund.stg.ynarcher.co.kr
management.stg.ynarcher.co.kr
```

dev 도메인 또는 Vercel preview:

```txt
dev-yna-work.vercel.app
preview deployments
```
...
NEXT_PUBLIC_APP_NAME=Y&A Work
NEXT_PUBLIC_APP_DOMAIN=work.ynarcher.co.kr
NEXT_PUBLIC_SUPABASE_URL=https://xxxx.supabase.co
NEXT_PUBLIC_SUPABASE_ANON_KEY=public-anon-key
NEXT_PUBLIC_AUTH_REDIRECT_URL=https://work.ynarcher.co.kr/auth/callback
```
...
```env
NEXT_PUBLIC_APP_ENV=production
NEXT_PUBLIC_APP_NAME=Y&A Hub
NEXT_PUBLIC_APP_DOMAIN=hub.ynarcher.co.kr
NEXT_PUBLIC_SUPABASE_URL=https://xxxx.supabase.co
NEXT_PUBLIC_SUPABASE_ANON_KEY=public-anon-key
NEXT_PUBLIC_AUTH_REDIRECT_URL=https://hub.ynarcher.co.kr/auth/callback
```
...
```env
NEXT_PUBLIC_APP_ENV=production
NEXT_PUBLIC_APP_NAME=Y&A Dev
NEXT_PUBLIC_APP_DOMAIN=dev.ynarcher.co.kr
NEXT_PUBLIC_SUPABASE_URL=https://xxxx.supabase.co
NEXT_PUBLIC_SUPABASE_ANON_KEY=public-anon-key
NEXT_PUBLIC_AUTH_REDIRECT_URL=https://dev.ynarcher.co.kr/auth/callback
...
Supabase Auth에는 각 앱의 callback URL을 등록해야 한다.

운영:

```txt
https://hub.ynarcher.co.kr/auth/callback
https://dev.ynarcher.co.kr/auth/callback
https://work.ynarcher.co.kr/auth/callback
https://mna.ynarcher.co.kr/auth/callback
https://project.ynarcher.co.kr/auth/callback
https://fund.ynarcher.co.kr/auth/callback
https://management.ynarcher.co.kr/auth/callback
```

staging:

```txt
https://hub.stg.ynarcher.co.kr/auth/callback
https://dev.stg.ynarcher.co.kr/auth/callback
https://work.stg.ynarcher.co.kr/auth/callback
https://mna.stg.ynarcher.co.kr/auth/callback
https://project.stg.ynarcher.co.kr/auth/callback
https://fund.stg.ynarcher.co.kr/auth/callback
https://management.stg.ynarcher.co.kr/auth/callback
```
...
```txt
apps/hub          -> localhost:3000
apps/dev          -> localhost:3001
apps/work         -> localhost:3002
apps/mna          -> localhost:3003
apps/project      -> localhost:3004
apps/fund         -> localhost:3005
apps/management   -> localhost:3006
```
...
```txt
apps/work/.env.example
apps/hub/.env.example
apps/dev/.env.example
```
...
서비스 간 이동은 도메인 링크로 처리한다.

예시:

```txt
Y&A Work에서 Hub 스타트업 상세 보기
-> https://hub.ynarcher.co.kr/startups/{startup_id}
```
...
```txt
Vercel 프로젝트를 생성했는가?
Root Directory가 apps/{service}로 설정되었는가?
도메인을 연결했는가?
