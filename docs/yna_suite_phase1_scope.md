# Y&ARCHER WORKS 1차 구축 범위 가이드

> 2026-07-04 아키텍처 개편: 다중 앱 → WORKS 단일 내부 앱 + GUEST 외부 앱. Hub→HUB 섹션, Dev→관리(ADMIN) 섹션, Work→AC 섹션. 도메인/스키마 키 dev→admin, work→ac.

본 문서는 Y&ARCHER WORKS의 실제 운영을 위한 **1차 구축 범위(Phase 1 Scope)**를 정의한다. 이 문서는 검증용 MVP가 아니라, 실제 업무 시스템을 단계적으로 구축하기 위한 릴리즈 기준이다.
...
Y&ARCHER WORKS는 실험용 MVP가 아니라 실제 운영 시스템으로 구축한다. 따라서 본 문서에서는 `MVP` 대신 `1차 구축`, `Phase 1`, `Core Foundation`이라는 표현을 사용한다.
...
Y&ARCHER WORKS는 한 번에 모든 섹션을 완성하지 않는다. 먼저 전사 공통 기반을 만들고, 이후 도메인 앱을 순차적으로 연결한다.
...
AC 섹션 실제 프로그램/신청 운영
FUND 섹션 실제 펀드 업무 운영
PROJECT 섹션 실제 프로젝트 업무 운영
M&A 섹션 실제 딜 업무 운영
MANAGEMENT 섹션 실제 HR/성과 업무 운영
외부 스타트업 포털
외부 전문가 평가 포털
고급 알림/메일 워크플로우
자동 병합
...
```txt
domain_name 목록에는 ac, fund, project, mna, management를 포함한다.
...
Supabase Auth 로그인
서비스별 도메인 접근 제어 구조
admin.user_permissions 기반 read/write 판단
권한 없음 페이지
읽기 전용 UI 처리
RLS 기본 정책
권한 변경 감사 로그
...
ac/fund/project/mna/management 도메인 권한은 관리 섹션에서 설정 가능해야 한다.
```
...
## 11. AC 연결 Mock/Test Flow (구 Work)
...
Phase 2 AC 개발 시 재설계를 줄인다.
```

검증 시나리오:

```txt
1. 관리 섹션에서 ac 도메인 권한을 가진 사용자 생성
2. Mock Work 프로그램 생성
...
## 12. Phase 2: AC 섹션 연결 (구 Y&A Work)
