# Y&A Suite 마스터 데이터 및 병합 정책 가이드

본 문서는 Y&A Hub의 마스터 데이터 생성, 식별, 중복 후보 판단, 병합, 이력 관리 정책을 정의한다.

Y&A Suite에서 Hub는 단순한 통합 목록이 아니라 전사 공통 자원에 대한 **신뢰 가능한 마스터 원장**이다. Work, M&A, Project, Fund, Management는 각자의 업무 데이터를 관리하지만, 스타트업/전문가/협력사 같은 공통 자원은 Hub 마스터를 참조한다.

## 1. 핵심 전제

Y&A 업무 특성상 스타트업, 전문가, 협력사는 사업자등록번호나 법인번호 같은 공식 식별자가 없는 상태로 유입되는 경우가 많다.

따라서 Y&A Suite의 식별 정책은 다음 전제를 따른다.
...
```sql
work.applications.startup_id -> hub.startups.id
...
Y&A Suite에서 식별자는 공식 번호만을 의미하지 않는다.
...
```txt
Work 화면에서 신규 스타트업 입력
-> hub.startups에 pending/temporary 마스터 생성
-> work.applications.startup_id가 해당 hub.startups.id 참조
-> Hub 병합 후보 큐에 올라감
```
...
Work 행사/프로그램 신청에서 같은 스타트업이 신청한다.
...
```txt
1. Work 신청 화면에서 회사명 입력
2. Hub 마스터를 먼저 검색
3. 기존 스타트업 후보를 추천
4. 사용자가 기존 스타트업을 선택
5. work.applications.startup_id = hub.startups.id 로 연결
6. Work 신청 이력이 기존 Hub 마스터의 생애 이력으로 쌓임
```

SQL 관점:

```sql
INSERT INTO work.applications (
    program_id,
    startup_id,
    applicant_user_id,
    application_status
...
3. work.applications는 일단 임시 startup_id에 연결
4. Hub가 기존 스타트업과 유사도 후보를 생성
5. Hub 관리자에게 병합 후보로 표시
6. 관리자가 확인 후 병합 승인
7. Work 신청 이력은 최종 Hub 마스터에 귀속
```
...
```txt
병합 전:
work.applications.startup_id = TEMP_ID

병합 후:
work.applications.startup_id = MASTER_ID
```
...
work.applications.startup_id = TEMP_ID 유지
```
...
### 10.3 Y&A 권장 방식: 혼합형 (최적화)

Y&A Suite는 혼합형을 기본으로 하되, **DB 락 병목 완화 정책**을 적용한다.
...
3. [실시간 Resolve] 비동기 동기화가 진행 중인 동안, 모든 비즈니스 조회 쿼리는 COALESCE(s.merged_into_id, s.id) 조인을 활용하여 실시간으로 최종 마스터 데이터를 보장한다.
...
[
  ...
  {
    "table": "work.program_participants",
...
Work, M&A, Project, Fund에서 새로운 자원을 입력할 때는 항상 Hub 검색을 먼저 수행한다.
...
```txt
program_activities와 meeting_minutes는 Work 실행 기록이다.
...
Y&A Suite의 마스터 데이터 정책은 다음을 기준으로 한다.
