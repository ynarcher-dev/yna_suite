...
본 문서는 Y&ARCHER WORKS의 1차 개발 스택을 정의하고, 각 기술을 선택하는 이유와 적용 범위를 정리한다. Y&ARCHER WORKS는 내부 통합 앱(WORKS)과 외부 포털(GUEST) 2개 앱으로 배포되며, 인증/권한/마스터 데이터는 하나의 체계로 공유하는 반응형 웹 기반 업무 시스템이다.

## 1. 기본 전제

Y&ARCHER WORKS는 다음 특성을 가진다.

1. 앱은 2개다. `apps/works`(내부 통합 앱)는 내부 7개 섹션(HUB/AC/FUND/PROJECT/M&A/MANAGEMENT/관리)을 권한 기반 메뉴로 구분하고, `apps/guest`(외부 포털)는 Phase 2에 구현한다(현재 placeholder).
2. 두 앱 모두 반응형 웹으로 개발한다.
...
Y&ARCHER WORKS는 `apps/works`, `apps/guest` 두 앱이 존재하고, 각 앱이 `packages/auth`, `packages/permissions`, `packages/master-data`, `packages/ui` 같은 공통 패키지를 공유한다.

`pnpm workspace`는 하나의 저장소 안에서 앱과 패키지 간 의존성을 명확하게 연결하기 좋다. 패키지 설치 속도와 디스크 사용량도 효율적이다.

`Turborepo`는 앱별 빌드, 테스트, 린트 작업을 빠르게 실행하고 캐싱할 수 있게 해준다. 앱별 독립 배포 구조에서는 특정 앱만 빌드하거나 테스트하는 일이 많기 때문에 유용하다.
...
Next.js는 앱 도메인, 로그인 콜백, 보호 라우트, 서버 측 처리, middleware를 관리하기 좋다. Y&ARCHER WORKS는 SSO, 권한, RLS, 권한 도메인(섹션)별 접근 제어가 중요하므로 단순 SPA보다 Next.js 기반이 안정적이다.

적용 기준:

```txt
apps/works   -> Next.js app (내부 통합 앱: HUB/AC/FUND/PROJECT/M&A/MANAGEMENT/관리 섹션)
apps/guest   -> Next.js app (외부 포털, Phase 2 구현 — 현재 placeholder)
```

## 5. Language: TypeScript

Y&ARCHER WORKS는 스타트업, 전문가, 협력사, 심사역, 펀드, 프로젝트, 권한 등 도메인 타입이 많다. TypeScript를 사용하면 도메인 모델과 DB 타입을 공통 패키지로 공유할 수 있고, 앱 간 데이터 계약을 명확하게 유지할 수 있다.
...
Supabase는 Y&ARCHER WORKS의 데이터 아키텍처와 잘 맞는다.

| Supabase 기능 | 사용 목적 |
| :--- | :--- |
| Auth | SSO 기반 계정, 로그인, 세션 |
| Postgres | 단일 물리 DB, 권한 도메인별 논리 스키마 분리 |
| RLS | 사용자별/도메인별 데이터 접근 제어 |
| Storage | 실사자료, 계약서, 첨부파일 |
| Edge Functions | 병합 처리, 알림, 배치성 작업 |

스키마는 권한 도메인(WORKS 내부 섹션)과 동일한 기준으로 나눈다. 도메인 키는 7종(hub, admin, ac, mna, project, fund, management)이다.
...
Y&ARCHER WORKS는 업무형 SaaS에 가깝다. 폼, 테이블, 모달, 탭, 필터, 상세 패널 같은 화면 요소가 반복적으로 등장한다.

Tailwind CSS는 섹션별 화면을 빠르게 만들면서도 디자인 규칙을 공통화하기 좋다. `packages/ui`에서 디자인 토큰과 공통 컴포넌트를 관리하면 섹션·앱 간 UI 일관성을 유지할 수 있다.
...
Y&ARCHER WORKS에서는 다음 흐름이 자주 발생한다.
...
Y&ARCHER WORKS의 주요 화면은 테이블 중심이 될 가능성이 높다.
...
TanStack Table은 headless 방식이라 Y&ARCHER WORKS의 디자인 시스템에 맞춰 자유롭게 UI를 입힐 수 있다. 정렬, 필터, 페이지네이션, 컬럼 숨김, 행 선택 같은 기능도 안정적으로 구현할 수 있다.
...
특히 Y&ARCHER WORKS는 권한과 화면 접근이 밀접하게 연결되므로, 핵심 플로우는 Playwright로 검증하는 것이 좋다.

## 13. Deploy: Vercel

Next.js를 사용한다면 Vercel이 가장 자연스럽다.

Vercel 프로젝트는 2개(works, guest)를 만들고, 각 프로젝트의 Root Directory를 해당 앱 폴더로 지정한다.
...
1차 범위는 모바일 앱이 아니라 반응형 웹이다. 내부 업무가 WORKS 단일 앱으로 통합되면서 추후 모바일 패키징 시 앱 진입점 문제도 단순해진다(내부용 진입점은 WORKS 하나).
...
Y&ARCHER WORKS의 1차 추천 스택은 다음과 같다.
...
이 조합은 내부 통합 앱(WORKS)과 외부 포털(GUEST)을 독립 배포하면서도, 공통 인증/권한/마스터 데이터 체계를 안정적으로 공유하기에 적합하다.
