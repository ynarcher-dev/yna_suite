# Y&ARCHER WORKS 디자인 시스템 가이드

> 2026-07-04 아키텍처 개편: 다중 앱/서브도메인 구조를 WORKS 단일 내부 앱(7개 섹션) + GUEST 외부 앱(Phase 2)으로 통합했다. 토큰/컴포넌트 규칙은 변경 없음.

본 문서는 Y&ARCHER WORKS의 공통 디자인 시스템 기준을 정의한다. 목표는 WORKS 앱 안의 섹션이 여러 개로 나뉘어도(그리고 GUEST 앱이 추가되어도) 사용자가 하나의 Y&A 업무 시스템을 쓰고 있다고 느끼게 하는 것이다.

Y&ARCHER WORKS의 UI는 **그레이스케일 중심의 업무형 인터페이스**를 기본으로 하며, 회사 CI 컬러인 `#E22213`은 활성화, 주요 액션, 포인트 강조에 제한적으로 사용한다.

## 1. 디자인 원칙

Y&ARCHER WORKS는 내부 업무, 투자, 프로젝트, 펀드, 마스터 데이터 관리를 위한 시스템이다. 따라서 장식적인 화면보다 빠른 판단과 안정적인 입력을 우선한다.

핵심 원칙:

1. 통일감: 모든 섹션은 같은 레이아웃, 컴포넌트, 상태 표현을 사용한다.
...
Y&ARCHER WORKS의 핵심 CI 컬러는 다음 두 가지이다.
...
Y&ARCHER WORKS의 대부분 화면은 아래 그레이스케일을 사용한다.
...
서비스별 컬러를 따로 강하게 만들지 않는다. Y&ARCHER WORKS의 정체성은 공통 그레이스케일과 CI Red에서 나온다.
...
```txt
HUB 섹션       -> Y&A Red accent
관리(ADMIN)    -> Gray + Red accent
AC 섹션        -> Gray + Red accent
M&A 섹션       -> Gray + Red accent
PROJECT 섹션    -> Gray + Red accent
FUND 섹션       -> Gray + Red accent
MANAGEMENT 섹션 -> Gray + Red accent
```
...
Y&ARCHER WORKS는 업무 밀도가 필요한 시스템이므로 지나치게 큰 여백을 피한다. 기본 spacing은 4px grid를 사용한다.
...
[packages/permissions 또는 apps/works(관리 섹션) 소유]
...
Y&ARCHER WORKS는 테이블 중심 업무가 많으므로 테이블 규격을 고정한다.
...
Y&ARCHER WORKS는 한국산업표준의 인간중심 설계, 사용성, 접근성 취지를 따르는 것을 목표로 한다. 실제 인증이나 공식 적합성 판단은 별도 심사 대상이므로, 본 문서는 제품 구현 기준으로 사용한다.
...
Y&ARCHER WORKS의 디자인 시스템은 다음 방향을 따른다.
