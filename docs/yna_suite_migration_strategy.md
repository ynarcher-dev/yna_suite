# Y&A Suite 데이터 마이그레이션 전략 가이드

본 문서는 기존 스타트업 DB, 엑셀/시트, 업무별 산재 데이터를 Y&A Hub 중심의 통합 데이터 구조로 이관하기 위한 전략을 정의한다.

Y&A Suite의 마이그레이션은 단순한 데이터 복사가 아니다. 기존 데이터를 Hub 마스터로 옮기면서 **출처, 식별 단서, 중복 후보, 변경 이력, 실패 이력**을 함께 남기는 과정이다.
...
Y&A Suite의 데이터 마이그레이션은 다음 목적을 가진다.
