...
--
-- 목적: 매 쿼리마다 dev.user_permissions 를 조인하는 성능 병목을 피하기 위해,
...
$$;
COMMENT ON FUNCTION dev.can_read_domain(TEXT) IS
...
$$;
COMMENT ON FUNCTION dev.can_write_domain(TEXT) IS
  'JWT 무조인 파싱 — 도메인 쓰기 권한(can_read 를 전제로 강제)';

-- 도메인 scope_type. 읽기 권한이 없으면 'none'.
CREATE OR REPLACE FUNCTION dev.get_scope_type(target_domain TEXT)
RETURNS TEXT
LANGUAGE SQL
STABLE
AS $$
    SELECT CASE
        WHEN dev.can_read_domain(target_domain) THEN COALESCE(
...
$$;
COMMENT ON FUNCTION dev.has_role(TEXT) IS
  'JWT app_metadata.roles 배열에 대상 역할 포함 여부(No-Join)';

-- 최고 관리자 여부. master 템플릿만 dev 접근을 받으므로 role 로 판정.
CREATE OR REPLACE FUNCTION dev.is_master()
RETURNS BOOLEAN
LANGUAGE SQL
STABLE
AS $$
    SELECT dev.has_role('master');
$$;
COMMENT ON FUNCTION dev.is_master() IS 'master 역할 보유 여부';

-- 마스터 병합 권한. Phase 1 은 hub 쓰기 권한자를 병합 가능자로 본다.
-- (rls_policy_matrix §9 "hub:write + merge 권한" — 세분화된 merge 플래그는 후속 Phase)
CREATE OR REPLACE FUNCTION dev.can_merge_master()
RETURNS BOOLEAN
LANGUAGE SQL
STABLE
AS $$
    SELECT dev.can_write_domain('hub');
$$;
COMMENT ON FUNCTION dev.can_merge_master() IS
  'Hub 마스터 병합 승인 권한(Phase 1 = hub:write)';

-- RLS 정책 표현식에서 호출 가능하도록 EXECUTE 부여.
GRANT USAGE ON SCHEMA dev TO authenticated, anon;
GRANT EXECUTE ON FUNCTION
    dev.can_read_domain(TEXT),
    dev.can_write_domain(TEXT),
    dev.get_scope_type(TEXT),
    dev.get_scope_id(TEXT),
    dev.has_role(TEXT),
    dev.is_master(),
