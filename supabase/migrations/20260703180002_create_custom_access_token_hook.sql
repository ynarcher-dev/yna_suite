...
--
-- 목적: access token 발급/갱신 시점마다 admin.user_permissions 를 읽어
...
--   uri = "pg-functions://postgres/admin/custom_access_token_hook" 로 등록한다.
-- 훅은 supabase_auth_admin 역할로 실행되므로 해당 역할에 실행/조회 권한을 부여한다.

CREATE OR REPLACE FUNCTION admin.custom_access_token_hook(event JSONB)
RETURNS JSONB
LANGUAGE plpgsql
AS $$
DECLARE
...
END;
$$;
COMMENT ON FUNCTION admin.custom_access_token_hook(JSONB) IS
  'Auth Hook: 토큰 발급/갱신 시 admin.user_permissions 를 app_metadata.permissions/roles 로 주입';

-- 훅 실행 주체(supabase_auth_admin)에 권한 부여, 일반 역할에는 노출 금지.
GRANT USAGE ON SCHEMA admin TO supabase_auth_admin;
GRANT EXECUTE ON FUNCTION admin.custom_access_token_hook(JSONB) TO supabase_auth_admin;
REVOKE EXECUTE ON FUNCTION admin.custom_access_token_hook(JSONB) FROM authenticated, anon, public;

-- 훅이 RLS(기본 deny) 하에서도 권한 원장을 읽을 수 있도록 조회 권한 + 전용 정책.
GRANT SELECT ON admin.user_permissions TO supabase_auth_admin;
CREATE POLICY "auth admin reads permissions"
    ON admin.user_permissions
    AS PERMISSIVE
    FOR SELECT
