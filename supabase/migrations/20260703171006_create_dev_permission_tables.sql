...
COMMENT ON TABLE dev.user_permissions IS '사용자×도메인 권한(No-Join JWT 캐싱의 원본)';

CREATE TRIGGER set_updated_at BEFORE UPDATE ON dev.user_permissions
...
COMMENT ON TABLE dev.permission_templates IS '권한 템플릿(역할별 도메인 권한/scope 기본값)';

CREATE TRIGGER set_updated_at BEFORE UPDATE ON dev.permission_templates
  FOR EACH ROW EXECUTE FUNCTION dev.set_updated_at();
...
COMMENT ON TABLE dev.permission_audit_logs IS '권한 변경 감사 로그 (수정/삭제 불가 전제)';

CREATE INDEX permission_audit_logs_target_idx ON dev.permission_audit_logs (target_user_id);
CREATE INDEX permission_audit_logs_actor_idx ON dev.permission_audit_logs (actor_user_id);
CREATE INDEX permission_audit_logs_created_at_idx ON dev.permission_audit_logs (created_at DESC);
