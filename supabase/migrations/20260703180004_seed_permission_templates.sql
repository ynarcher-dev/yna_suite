...
INSERT INTO admin.permission_templates (role_key, display_name, description, permissions, is_system, status)
VALUES
  ('master', '최고 관리자',
   '전 서비스와 권한 관리 가능',
   jsonb_build_object(
     'hub',        jsonb_build_object('can_read', true, 'can_write', true,  'scope_type', 'global'),
     'admin',        jsonb_build_object('can_read', true, 'can_write', true,  'scope_type', 'global'),
     'ac',       jsonb_build_object('can_read', true, 'can_write', true,  'scope_type', 'global'),
  ...
  ('executive', '경영진',
   ...
   jsonb_build_object(
     ...
     'ac',       jsonb_build_object('can_read', true, 'can_write', false, 'scope_type', 'global'),
  ...
  ('guest_expert', '외부 전문가',
   '배정된 평가/멘토링 중심',
   jsonb_build_object(
     'ac',       jsonb_build_object('can_read', true, 'can_write', false, 'scope_type', 'self')
   ), TRUE, 'active'),

  ('guest_startup', '참가 스타트업',
   '자기 회사/신청 정보 중심',
   jsonb_build_object(
     'ac',       jsonb_build_object('can_read', true, 'can_write', true,  'scope_type', 'company')
   ), TRUE, 'active'),
