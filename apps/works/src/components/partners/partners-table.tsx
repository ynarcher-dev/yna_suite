 ...
 * 컬럼: master_code/name/partner_type/business_number/대표자/검증/상태.
 * 사업자번호·대표자명 마스킹, 검색·유형·상태·검증 필터, 정렬, 페이지네이션.
 */
const PAGE_SIZE = 10;
...
export function PartnersTable({ partners }: { partners: PartnerMaster[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    const rows = partners.filter((p) => {
      if (ptype && p.partnerType !== ptype) return false;
      if (status && p.status !== status) return false;
      if (verif && p.verificationStatus !== verif) return false;
      if (query) {
        const hay = `${p.name} ${p.masterCode} ${p.representativeName ?? ""}`.toLowerCase();
    ...
    return rows;
  }, [partners, q, ptype, status, verif, sort]);
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      >
        ...
        <Select
          value={ptype}
          onChange={(e) => resetPage(setPtype)(e.target.value)}
          className="w-36"
          aria-label="기관유형 필터"
        >
      ...
      {filtered.length === 0 ? (
        <EmptyState title="협력사가 없습니다" description="검색 조건을 바꾸세요." />
      ) : (
        <>
          <Table>
            <THead>
            ...
            <TBody>
              {rows.map((p) => {
                const v = verificationMeta(p.verificationStatus);
                const st = masterStatusMeta(p.status);
                return (
                  <TR key={p.id} interactive onClick={() => router.push(`/partners/${p.id}`)}>
                    <TD>
                      <MasterCodeBadge code={p.masterCode} />
                    </TD>
                    <TD className="font-medium text-gray-900">{p.name}</TD>
                    <TD className="hidden text-gray-600 sm:table-cell">{partnerTypeLabel(p.partnerType)}</TD>
                    <TD className="hidden text-gray-600 lg:table-cell">
                      {p.businessNumber ? maskBusinessNumber(p.businessNumber) : "—"}
                    </TD>
                    <TD className="hidden text-gray-600 lg:table-cell">
                      {p.representativeName ? maskName(p.representativeName) : "—"}
                    </TD>
                    <TD>
                    ...
                    <TD className="hidden text-gray-600 md:table-cell">{fmtDate(p.updatedAt)}</TD>
                  </TR>
                );
              })}
            </TBody>
