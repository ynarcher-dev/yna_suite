...
*(근거: yna_suite_hub_admin_functional_spec.md §15~19, yna_suite_api_contracts.md §16~18)*
...
*(근거: yna_suite_hub_admin_functional_spec.md §4~7)*
...
*(근거: yna_suite_hub_admin_functional_spec.md §8~9)*
...
*(근거: yna_suite_migration_strategy.md, yna_suite_hub_admin_functional_spec.md §14)*
...
### [x] Phase 1.14 설계 문서 정합성 보완
*(근거: docs_jm/6_docs_review.md — 2026-07-04 문서 교차 리뷰)*
> 진행: 36건(높음 4·중간 15·낮음 17) 전부 보완 완료(2026-07-04). 기획자 방향 결정 4건(외부 사용자 지금은 잠금 / 임시 마스터 화면 신설 / 전건 처리 / 개인정보 삭제 절충안) 반영, docs 15개 + docs_jm 3개 수정, `/temporary-masters` 라우트 신설(nav 404 해소). typecheck/lint 20 태스크·테스트 3패키지·hub 빌드+smoke 통과. 문서 후속 3건(식별자 부분 unique·experts.user_id migration, guest_expert 제출 방식)은 Phase 2 전 처리 — 4_memo 이슈31.

*   **[x] `docs_jm/6_docs_review.md`를 참고하여 문서 간 불일치 보완**
...
### [ ] 테스트 게이트
*(근거: yna_suite_hub_admin_functional_spec.md §23, yna_suite_tech_stack.md §12, yna_suite_maintenance_rules.md §8)*
...
*(근거: yna_suite_phase1_scope.md §14, yna_suite_hub_admin_functional_spec.md §22)*
...
*   **[ ] 권한 없는 사용자 차단 및 외부 사용자 Hub/Dev 접근 차단 테스트 통과** *(외부 사용자의 self/company scope 격리 검증은 Phase 2 외부 포털 범위 — phase1_scope §12)*
