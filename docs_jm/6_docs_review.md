...
*   **[ ] H1. `migration_strategy`의 staging 테이블 DDL이 `data_model` 최신본(Phase 1.12 반영)보다 뒤처짐**
...
*   **[ ] H2. `guest_expert` 권한 충돌 — 권한표는 work 읽기 전용(R), RLS 매트릭스는 평가 쓰기 허용**
...
*   **[ ] H3. `guest_startup` 권한 충돌 — 권한표는 hub 접근 None, RLS 매트릭스는 `hub.startups` 제한 SELECT/UPDATE 허용**
...
*   **[ ] H4. "Temporary Masters(임시 마스터)" 화면 — IA는 Phase 1 필수인데 기능 명세 부재 + 실제로는 사이드바 링크만 있는 404 상태**
...
*   **[ ] M2. `hub.merge_events.sync_status` 컬럼이 master_data_policy DDL에 없음**
...
*   **[ ] M3. import row 실패 상태값 목록 불일치 — 스키마 comment가 허용하지 않는 상태를 migration_strategy가 사용**
...
*   **[ ] M4. `master_identifiers` 단일 unique 인덱스 vs 식별자 유형별 차등 unique 정책 충돌**
...
*   **[ ] M5. alias "inactive 상태 우선" 권고 vs `hub.master_aliases`에 상태 컬럼 부재**
...
*   **[ ] M6. `staging` 스키마가 공식 스키마 목록(7개)에 없음**
...
*   **[ ] M7. `management_office`의 `hub.managers` 쓰기 근거 없음**
...
*   **[ ] M8. `hub.experts`에 auth 계정 연결 컬럼 없음 — guest_expert self scope RLS 구현 불가**
...
*   **[ ] M9. `master_data_merge` 세분 권한이 권한 모델에 없음**
...
*   **[ ] M10. 전문가/협력사 마스터 "생성"이 범위 문서에는 Phase 1 필수, 기능 명세에는 없음**
...
*   **[ ] M11. "기본 감사 로그 조회" 화면이 범위·IA에는 Phase 1 필수인데 기능 명세에 화면 섹션 없음**
...
*   **[ ] M12. 외부 사용자(guest) 격리 테스트의 Phase 배치 충돌**
...
*   **[ ] M13. UI 컴포넌트 전략 드리프트 — 공식 문서는 shadcn/ui(Radix) 전제, 실결정은 "외부 패키지 없이 네이티브 구현"**
...
*   **[ ] M14. 환경변수 문서 드리프트 — `NEXT_PUBLIC_COOKIE_DOMAIN`이 기준 문서에 없음**
...
*   **[ ] M15. 브랜치/커밋 프로세스 충돌 — PR 기반 merge vs 단위 작업 즉시 commit & push**
...
*   **[ ] L1. `merge_candidates`의 `on_hold`(보류) 노출 불일치**
...
*   **[ ] L2. `scope_type: "department"`가 데이터 모델에 미정의**
...
*   **[ ] L3. 권한 조회 API 응답의 role_key 구조 모순**
...
*   **[ ] L4. SQL 관리 위치 이원화 — `supabase/schemas/`·`policies/` 구조 vs migrations 단일 경로**
...
*   **[ ] L5. smoke 테스트 계정 목록 불일치 (7종 vs 10종)**
...
*   **[ ] L6. staging 도메인 표기 자기모순 (environment_deployment 내부)**
...
*   **[ ] L7. `dev.permission_audit_logs`의 `request_id` 컬럼 유무 불일치**
...
*   **[ ] L8. `can_write ⇒ can_read` 강제 수준 불일치**
...
*   **[ ] L10. 병합 resolve 메커니즘 서술 불일치 (DB 함수 vs view)**
...
*   **[ ] L15. Phase 1 구현 순서 불일치 (import 단계 배치)**
